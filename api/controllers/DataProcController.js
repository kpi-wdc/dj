...
module.exports = {
  ...
  process1: function(req, res){
       logger.debug("Process for "+JSON.stringify(req.body))
      // Cache
      //   .get(req.body)
      ...
      //     }else{
            if (req.body.data_query){
              var script = "src(ds:'{{id}}');query({{query}});save()"
              var context = {
                  id : req.body.data_query.datasetID,
                  query: JSON.stringify(req.body.data_query.query)
              }
              var locale = (req.body.data_query.locale === "uk") ? "ua" : req.body.data_query.locale;
              execScript(scriptParser.applyContext(script,context), locale)
                .then(function(result){
                  return res.send(result.table)
                })
              ...
              if(req.body.data_id) {
                var script = scriptParser.applyContext("src(t:'{{data_id}}');save()", req.body);
                execScript(script)
                  .then(function(result){
                    return res.send(result.table)
                  })
                ...
                //       logger.debug("PARAMS"+ JSON.stringify(p))
