...
define(["angular", "angular-oclazyload", "angular-nvd3", "/widgets/data-util/adapter.js"], function (angular) {
  ...
  m.factory("NVD3Widget", ["$http", "$ocLazyLoad", "APIProvider", "APIUser", "adapter", "pageSubscriptions", function ($http, $ocLazyLoad, APIProvider, APIUser, adapter, pageSubscriptions) {
    $ocLazyLoad.load({
      files: ["/components/nvd3/nv.d3.css", "/widgets/nvd3-widget/nvd3-widget.css"]
    });
    ...
    var NVD3Widget = function ($scope, params) {
      ...
      $http.get(params.optionsURL).success(function (data) {
        $scope.options = data;
        ...
        $scope.settings.options = $scope.options;
      });
      ...
      $scope.APIProvider.config(function () {
        ...
        if ($scope.widget.data && $scope.widget.data.standalone) {
          $scope.series = params.serieAdapter.getSeries ? params.serieAdapter.getSeries(adapter.getData($scope, params.serieGenerator)) : adapter.getData($scope, params.serieGenerator);
          $scope.settings = { options: $scope.options, data: $scope.series };
          return;
        }
