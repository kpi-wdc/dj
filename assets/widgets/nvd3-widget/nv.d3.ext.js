  ...
  nv.models.map = function () {
    ...
    function chart(selection) {
      selection.each(function (data) {
        ...
        var data1 = data[0].filter(function(item){return !item.disabled != undefined && item.disabled == false})[0];
        console.log(data1)
        if (!data1){
          data1 = data[0][0];
        }
        ...
        wrap.select( '.nvd3').exit().remove();
        var wrapEnter = wrap.enter().append('g').attr('class', 'nvd3 nv-wrap nv-map nv-chart-' + id);

        var gEnter = wrapEnter.append('g');
        var g = wrap.select('g');

        gEnter.append('g').attr('class', 'nv-map');
        //gEnter.append('g').attr('class', 'nv-pieLabels');

        wrapEnter.attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
        //g.select('.nv-map').attr('transform', 'translate(' + availableWidth / 2 + ',' + availableHeight / 2 + ')');
        ...
        var geoData = topojson.feature( worldTopo, worldTopo.objects[ 'world' ] ).features
          .filter(function(feature) {
            return feature.id !== "ATA";
          })
        //.filter(function(features){
        //  if(!data1) return false;
        //  if(data1.values.filter(function(item){
        ...
        var projection = d3.geo.mercator()
          .scale((sc)/2/Math.PI)
          .translate([availableWidth / 2, availableHeight / 1.3]);
