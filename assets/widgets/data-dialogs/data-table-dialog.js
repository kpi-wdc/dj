...
define(["angular", "widgets/data-util/keyset", "widgets/data-util/adapter", "angular-foundation", "widgets/data-dialogs/palettes1"], function (angular) {
  var m = angular.module("app.widgets.data-dialogs.data-table-dialog", ["app.widgets.data-util.keyset", "app.widgets.data-util.adapter", "mm.foundation", "app.widgetApi", "app.widgets.palettes1"]);

  m.factory("DataTableDialog", ["KeySet", "TableGenerator", "ScatterSerieGenerator", "$modal", "APIUser", "APIProvider", "pageSubscriptions", "pageWidgets", "Palettes1", "Normalizer", function (KeySet, TableGenerator, ScatterSerieGenerator, $modal, APIUser, APIProvider, pageSubscriptions, pageWidgets, Palettes1, Normalizer) {

    var DataTableDialog = function DataTableDialog(scope) {

      this.scope = scope;
      this.conf = {};
      this.dsList = pageWidgets().filter(function (item) {
    ...
    DataTableDialog.prototype = {
      ...
      setColor: function setColor(palette) {
        if (angular.isDefined(palette)) this.conf.decoration.color = this.inverseColor ? this.inverse(palette) : palette;
      },

      RGBA: function RGBA(hex, opacity) {
        if (angular.isUndefined(hex)) {
      ...
      getCellBackground: function getCellBackground(row, col, value) {
        if (angular.isUndefined(value)) {
          return "transparent";
        }if (this.conf.decoration.colorize == false) {
          return "transparent";
        ...
        return this.RGBA(this.conf.decoration.color[index], this.conf.decoration.opacity ? this.conf.decoration.opacity : 0.5);
      },

      getCellStyle: function getCellStyle(row, col, value) {
      ...
      calculateRanges: function calculateRanges() {
        ...
        for (var i in this.table.header.body) {
          var t = this.table;
          var values = this.table.body.map(function (item) {
      ...
      inverseColor: function inverseColor(palette) {
        if (angular.isDefined(palette)) this.conf.decoration.color = this.inverse(palette);
      },

      inverse: function inverse(palette) {
      ...
      dataValue: function dataValue(arg) {
      ...
      setEnabled: function setEnabled(steps) {
        for (var i in steps) steps[i].disabled = false;
      },

      setDisabled: function setDisabled(steps) {
        for (var i in steps) steps[i].disabled = true;
      },

      getItemStyle: function getItemStyle(dim, cat) {
      ...
      getDatasetStyle: function getDatasetStyle(dataset) {
      ...
      restoreState: function restoreState(conf, provider) {

        if (conf && conf.standalone) {
          this.table = conf.table;
          this.conf.standalone = true;
          this.setDisabled(this.steps.filter(function (item, index) {
        ...
        if (!conf) {
          return;
        }if (!conf.selectedDataset) {
          return;
        }this.setState(2, this.conf.metadata.find(function (item) {
        ...
        for (var i in this.conf.selectedDataset.dimensions) {
          ...
          for (var j in conf.selection[i].collection) {
            this.selectCategory(this.conf.selectedDataset.dimensions[i], this.conf.selectedDataset.dimensions[i].categories.find(function (item) {
              return item.id == conf.selection[i].collection[j].id;
            }));
          }
        }

        if (!this.readyForDataFetch()) {
          return;
        }this.setState(3);
        if (this.table) {
      ...
      appendIfNotExist: function appendIfNotExist(subscription) {
      ...
      removeIfExist: function removeIfExist(subscription) {
        var subscriptions = pageSubscriptions();
        for (var i in subscriptions) {
          if (subscriptions[i].emitter == subscription.emitter && subscriptions[i].receiver == subscription.receiver) {
      ...
      sortTable: function sortTable(header, item) {
        if (item) {
      ...
      changeOrder: function changeOrder(header, item) {
        if (item) {
      ...
      readyForSerieGeneration: function readyForSerieGeneration() {
        var result;
        if (this.table.header.coordX) {
          return true;
        }for (var i in this.table.header.body) {
          if (this.table.header.body[i].coordX) {
            return true;
          }
        }
        return false;
      ...
      setCoordX: function setCoordX(header, item, rowField) {
      ...
      setState: function setState() {
        switch (arguments[0]) {
          ...
          case 1:
            ...
            this.setEnabled(this.steps.filter(function (item, index) {
              return index < 2;
            }));
          ...
          case 2:
            // select dataset , dimension items selection in process
          ...
          case 3:
            // get data from dataset provider, fields role selection in process
      ...
      autoselect: function autoselect(dimension) {
      ...
      selectCategory: function selectCategory(dimension, category) {
      ...
      selectAllCategories: function selectAllCategories(dimension) {
      ...
      unselectAllCategories: function unselectAllCategories(dimension) {
        dimension.selection.set([]);
        this.setState(2);
      },

      readyForDataFetch: function readyForDataFetch() {
        var test = {
          rows: false,
          columns: false,
          allRole: true,
      ...
      setDimensionRole: function setDimensionRole(dimension, role) {
        var dims = this.conf.selectedDataset.dimensions;
        switch (role) {
          case "Fix Value":
            dimension.selection.role = "Fix Value";
          ...
          case "Rows":
            for (var i in dims) {
              if (dims[i].selection.role && dims[i].selection.role == role) {
  ...
  m.controller("DataTableConfigDialog", ["$scope", "$modalInstance", "widgetScope", function ($scope, $modalInstance, widgetScope) {
...
/*angular.extend(data, $scope.basicProperties)*/ //define(["angular","widgets/data-util/keyset", "widgets/data-util/table-adapter",'angular-foundation'],
//    function (angular) {
...
//
//    })
//# sourceMappingURL=../data-dialogs/data-table-dialog.js.map