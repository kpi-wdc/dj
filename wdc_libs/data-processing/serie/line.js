...
module.exports = function(table,params){
  ...
  var $axisX = {
   ...
   var keys = new query()
                    .from(result.map(function(serie){return serie.key}))
                    .distinct()
                    .get();
  needIndex = keys.length < result.length;
   ...
   result.forEach(function(serie,index){
    ...
    serie.values = serie.values
                    .filter(function(item){return item.category == serie.category})
                    .filter(function(item){return item.y != null})
   
     if($axisX.role == "time"){
      serie.values = new query()
                        .from(serie.values)
                        .orderBy(query.criteria["Date"]["A-Z"](function(item){return item.x}))
                        .get()
     }
   })
  return result; 
